...
MAX_CONCURRENT_TOPIC_REQUESTS = 10
...
def _extract_topics_full(
    transcript_text: str, model: str, api_key: str | None, trace_id: str
) -> list[str]:
...
def _extract_topics_chunk(
    chunk_text: str, model: str, api_key: str | None, trace_id: str
) -> list[str]:
...
def _merge_topic_lists(
    segment_topic_lists: list[list[str]], model: str, api_key: str | None, trace_id: str
) -> list[str]:
...
def extract_topics(
    ...
    *,
    ...
    if total_chars <= TOPIC_EXTRACTION_MAX_CHARS:
        logger.info("%sTopic extraction: Option A (full transcript, %d chars)", prefix, total_chars)
        transcript_text = _format_lines_for_prompt(lines)
        return _extract_topics_full(transcript_text, model, api_key, trace_id)

    # Option B: chunk and merge
    chunks = slice_for_llm(lines)
    logger.info(
        "%sTopic extraction: Option B (chunked, %d chars, %d chunks, workers<=%d)",
        prefix,
        total_chars,
        len(chunks),
        MAX_CONCURRENT_TOPIC_REQUESTS,
    )
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {
            executor.submit(_extract_topics_chunk, ct, model, api_key, trace_id): i
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.warning("%sTopic extraction failed for chunk %s: %s", prefix, idx, e)
    # Restore order
    results.sort(key=lambda x: x[0])
    segment_topic_lists = [t for _, t in results]
    if not segment_topic_lists:
        return []
    return _merge_topic_lists(segment_topic_lists, model, api_key, trace_id)